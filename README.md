...
PhpTranspilerは、TypeScriptからPHPへの変換を担当する独立ビルドツールである。

実装正本はTypeScriptとし、実行ランタイムはDenoのみとする。Node.js、npm、package-lockファイル、CDN依存は通常の実行ランタイムおよび依存物として使用しない。

ローカル検査はDenoで実行する。Deno runnerは生成PHPに対して `php -l` を実行するため、標準検査にはPHP CLIも必要である。

Denoを使用できない場合に限り、依存なしNode.jsフォールバックでゴールデン出力一致を確認できる。この経路は開発・検査用途に限定し、PHP構文検証はDeno runnerまたは `Tools/check/check-phptranspiler.sh` で実行する。Node.jsおよびnpm依存物をリリースパッケージまたは利用者実行ファイルに含めてはならない。
...
PhpTranspiler pt.0.59は、PHP 8.4以降の安全・安定サブセットを対象とする。PHP 8.5以降で非推奨・廃止・高リスクとなる構文/APIは生成しない。PHP標準ライブラリは、必要なものだけを `Php.*` helperとして明示許可する。
...
- PHPファイルディレクティブ: `declare(strict_types=1)`、`namespace`、`use`、定数未定義時のBootstrap実行入口
...
この全体検査は、必須ファイル、バージョン形式、`Docs/` 構成、Node.js/npmプロジェクトファイル不在、Deno test task、golden/errorテスト、PHP CLIの存在を確認する。
